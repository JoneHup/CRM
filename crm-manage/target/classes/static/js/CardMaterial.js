		...
		return {
			...
			privates : {
				...
				_createCard : function (data, i) {
					var _html = '<div class="materialCardItem clearfix">'
									+'<dl class="left materialCardDl">'
										+'<dt class="clearfix">'
											+'<span class="left materialName">'+data.title+'</span>'
											+'<em class="left materialClassify">'+data.type+'</em>'
										+'</dt>'
										+'<dd>'+data.content+'</dd>'
										
									+'</dl>'
									+'<a href="#" title="删除" class="right deleteLink_1"></a>'
									+'<a href="#" title="编辑" class="right editLink_1"></a>'
								+'</div>';
				    return _html;	
				},
				/**
				...
				_bindCallBacks:function(){
					var that = this,
						class_card = 'materialCardItem',
						$container = $('#' + this.id);
						
					//卡片：删除
					$container.off('click','#'+that.id+' .deleteLink_1').on('click','#'+that.id+' .deleteLink_1',function (ev){
						var card = $(this).closest('.'+class_card),
							index = $('#' + that.id).find('.'+class_card).index(card);
						that.callBacks.materialDelete.apply(card,[ev,that.data.data[index]]);
//						var data = that.data.data[index];
//						alert(data.id);						
					});
