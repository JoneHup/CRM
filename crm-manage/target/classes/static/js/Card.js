			...
			*/
			...
			operation : function(){
				...
				if (this.usepage !== null) {
					if (this.usepage.type == 1) {
						...
						!this.usepage.pageSizeAble&&(this.usepage.pageSizeAble=false);//设置分页默认样式
						this.pageWithNumFnc();
					}
				}
			...
			privates : {
                ...
                _reloadData : function(query){
                	//当关键字为空时候，点击查询，即为查询全部数据。
                	//否则的是 模糊查询
                	var keyword = $.trim($(".searchBox input").val()); //jquery 获取输入框关键词值          
